...
class BookModel:
    ...
    def count(search=None, category_id=None, available_only=False):
        """Count books matching the filter criteria."""
    ...
    def create(data):
        """Insert a new book."""
        query = """
            INSERT INTO books (title, author, isbn, category_id, edition, publisher,
                               publish_year, total_copies, available_copies, cover_image, description)
        ...
        params = (
            ...
            data.get('total_copies', 1), data.get('total_copies', 1),
    ...
    def update_available_copies(book_id, change):
        """Increment or decrement available copies by `change` (+1 or -1)."""
    ...
    def get_low_stock(threshold=2):
        """Get books with available copies at or below the threshold."""
        query = """
            SELECT b.*, c.name AS category_name
            FROM books b
            LEFT JOIN categories c ON b.category_id = c.id
